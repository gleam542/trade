...
畫面上有兩塊容易搞混、但範圍不同的區塊：
- **策略試算／建議商品**：呼叫 `/api/advise`，永遠掃描**全部**追蹤中的交易對，跟你目前點開哪個分頁無關——它可能推薦一個你根本沒在看的交易對。
- **最新判斷**（摘要條，在策略試算下方）：只反映你**目前選取的分頁**（例如點 BTCUSDT 就顯示 BTCUSDT 自己的 `generate_signal()` 結果），切換分頁就會跟著換。
