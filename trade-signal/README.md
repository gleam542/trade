...
**策略試算**——輸入本金、目標盈利與預計小時數，掃描全部追蹤中的交易對，挑出訊號方向與信心率，附上那個方向的歷史勝率、平均每小時報酬率，以及參考進場區間、止損與目標價（都是 ATR 的算術）。目標高於歷史節奏時會直接說出來，樣本太少的候選也會標明排除了幾個。下方橫條圖把每個候選都跟目標畫在一起比較：

![策略試算](docs/console.png)

**交易對**——價格圖疊上布林通道、EMA100 趨勢線與費波那契回撤位，三角形標出每根 K 線當下會發出的訊號（綠色做多、紅色做空）；滑鼠移上去顯示那一根的所有指標讀數，四張圖的十字線同步。下方還有 RSI、KD、MACD 三張子圖與逐根數值的原始資料表：

![價格圖與指標](docs/chart.png)
